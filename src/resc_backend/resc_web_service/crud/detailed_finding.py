...
def get_detailed_findings(
    ...
    if findings_filter.rule_tags:
        rule_tag_subquery = rule_tag_subquery.filter(
            model.DBtag.name.in_(findings_filter.rule_tags)
        )
    if findings_filter.rule_pack_versions or findings_filter.rule_names:
        ...
        if findings_filter.rule_pack_versions:
            rule_tag_subquery = rule_tag_subquery.filter(
                model.DBrule.rule_pack.in_(findings_filter.rule_pack_versions)
            )
        if findings_filter.rule_names:
            rule_tag_subquery = rule_tag_subquery.filter(
                model.DBrule.rule_name.in_(findings_filter.rule_names)
            )
    ...
    findings: List[detailed_finding_schema.DetailedFindingRead] = (
        query.offset(skip).limit(limit_val).all()
    )
...
def get_detailed_finding(
    ...
    max_scan_subquery = db_connection.query(
        model.DBscanFinding.finding_id,
        func.max(model.DBscanFinding.scan_id).label("scan_id"),
    )
    max_scan_subquery = max_scan_subquery.group_by(
        model.DBscanFinding.finding_id
    ).subquery()
