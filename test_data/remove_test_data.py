...
try:
    ...
    DB_CONNECTION_STRING = (
        f"DRIVER={{{MSSQL_ODBC_DRIVER}}};SERVER={MSSQL_DB_SERVER};DATABASE={MSSQL_SCHEMA};"
    ...
    try:
        sql_file_path = "test_data/drop_tables.sql"
        with open(sql_file_path) as sql_file:
            sql_script = sql_file.read()
        cursor.execute(sql_script)
        conn.commit()
        logging.info("removed all tables from DB")
    except pyodbc.Error as ex:
        logging.error(f"Query execution failed while clearing database: {ex}")
        sys.exit(1)
    finally:
        cursor.close()
        conn.close()
    ...
    logging.error(f"An error occurred while clearing database: {ex}")
    sys.exit(1)
