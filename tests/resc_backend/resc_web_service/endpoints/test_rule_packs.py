...
class TestRules(unittest.TestCase):
    ...
    def test_get_rules_without_rule_name(self, get_rule_by_rule_name_and_rule_pack_version):
        with self.client as client:
            response = client.get(f"{RWS_VERSION_PREFIX}{RWS_ROUTE_RULE_PACKS}/1.0.1/rules")
            assert response.status_code == 422, response.text
            data = response.json()
    ...
    def test_get_rules_with_rule_name_and_rule_pack(self, get_rule_by_rule_name_and_rule_pack_version):
        ...
        with self.client as client:
            response = client.get(f"{RWS_VERSION_PREFIX}{RWS_ROUTE_RULE_PACKS}/1.0.1/rules?rule_name=rule_name_1")
            assert response.status_code == 200, response.text
            data = response.json()
