...
/bin/sh -c "uv run alembic upgrade head && uv run python3 ./test_data/insert_test_data.py && uv run uvicorn resc_backend.resc_web_service.api:app --workers 1 --host 0.0.0.0 --port $RESC_API_PORT"
